...
// ─── Get user_id from wherever the app stored it after login ──────────────────
...
function FavCard({ spot, onRemove }: { spot: FavoriteSpot; onRemove: (spot: FavoriteSpot) => void }) {
  ...
  return (
    ...
    >
      {imgSrc && (
        <img src={imgSrc} alt={spot.spot_name} style={{ width: "100%", height: 190, objectFit: "cover", display: "block" }} />
      )}
      <div style={{ padding: "1.25rem", flex: 1, display: "flex", flexDirection: "column", gap: 6 }}>
        ...
        <div style={{ marginTop: 6 }}>
          <button
            ...
            style={{ background: "#fff", border: "1px solid #fca5a5", color: "#dc2626", borderRadius: 6, padding: "5px 14px", fontSize: 12, fontWeight: 500, cursor: "pointer", fontFamily: "'DM Sans', sans-serif", display: "inline-flex", alignItems: "center", gap: 6, transition: "background 0.15s" }}
      ...
      {[1, 2, 3].map(i => (
        <div key={i} style={{ background: "#fff", borderRadius: 16, border: "1.5px solid #bfdbfe", overflow: "hidden" }}>
          ...
          <div style={{ padding: "1.25rem", display: "flex", flexDirection: "column", gap: 10 }}>
            <div style={{ height: 14, width: "40%", background: "#f1f5f9", borderRadius: 8, animation: "pulse 1.5s ease-in-out infinite" }} />
            <div style={{ height: 18, width: "70%", background: "#f1f5f9", borderRadius: 8, animation: "pulse 1.5s ease-in-out infinite" }} />
            <div style={{ height: 13, width: "90%", background: "#f1f5f9", borderRadius: 8, animation: "pulse 1.5s ease-in-out infinite" }} />
            <div style={{ height: 13, width: "55%", background: "#f1f5f9", borderRadius: 8, animation: "pulse 1.5s ease-in-out infinite" }} />
          </div>
        </div>
      ))}
    </div>
          ...
          <div style={{ padding: "1.5rem", flex: 1, fontFamily: "'DM Sans', sans-serif" }}>
            ...
            <div style={{ background: "#fff", borderRadius: 16, padding: "1.1rem 1.5rem", display: "inline-flex", alignItems: "center", gap: "1rem", border: "1.5px solid #bfdbfe", marginBottom: "2rem", boxShadow: "0 2px 8px rgba(26,86,219,0.07)" }}>
              ...
              <div>
                ...
                <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 28, color: "#1e3a8a", lineHeight: 1 }}>
                ...
                {loading ? (
                  ...
                  <>
                    ...
                    {filtered.length === 0 ? (
                      ...
                      <div className="fav-grid">
                        {filtered.map(spot => (
                          <FavCard key={spot.spot_id} spot={spot} onRemove={setRemoveTarget} />
                        ))}
                      </div>
                    )}
                  </>
